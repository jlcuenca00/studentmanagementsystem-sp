window.Utils = (() => {
  ...
  function computeGradeAverage(record) {
    return round((Number(record.prelim) + Number(record.midterm) + Number(record.final)) / 3);
  }
  ...
  function gradesForStudent(studentId) {
    return window.AppState.grades.filter(item => item.studentId === studentId);
  }

  function gradesForClass(classId) {
    return window.AppState.grades.filter(item => item.classId === classId);
  }

  function announcementsForStudent(student) {
    return window.AppState.announcements.filter(item => item.target === "General" || student.classIds.includes(item.target));
  }
  ...
  function calendarData(events, monthDate = new Date("2026-04-01")) {
    ...
    return {
      year,
      month,
      monthLabel: monthDate.toLocaleDateString("en-PH", { month: "long", year: "numeric" }),
      weekdays: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
      startWeekday,
      totalDays,
      eventsByDay,
      todayDay: today.getFullYear() === year && today.getMonth() === month ? today.getDate() : null
    };
  }
